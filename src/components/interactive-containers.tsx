...
export function InteractiveContainers() {
  ...
  return (
    <div className="flex gap-4 w-full h-full p-1">
      <div 
        className={`h-full rounded-xl bg-gradient-to-br w-[70%] from-muted/40 via-muted/50 to-muted/40 transition-all duration-300 ease-in-out cursor-pointer overflow-hidden backdrop-blur-sm shadow-lg hover:shadow-xl ${
      ...
      >
        <div className="relative h-full">
          ...
          <div className={`h-full flex flex-col transition-all duration-300 ${
            focusedContainer !== 'right' ? 'opacity-100 translate-x-0' : 'opacity-70'
          }`}>
            ...
            <div className={`p-4 border-t border-white/5 transition-all duration-300 ${
      ...
      <div 
        className={`rounded-xl bg-gradient-to-br from-muted/50 via-muted/50 to-muted/60 transition-all duration-300 ease-in-out cursor-pointer overflow-hidden backdrop-blur-sm shadow-lg hover:shadow-xl ${
          focusedContainer === 'right' ? 'w-[70%] ring-2 ring-primary/20 bg-primary/20' : focusedContainer === 'left' ? 'w-[30%]' : 'w-1/2'
        }`}
        onClick={() => setFocusedContainer('right')}
      >
        <div className="relative h-full">
          <div className="absolute inset-0 bg-black/20 backdrop-blur-[2px]" />
          <div className={`absolute inset-0 ${focusedContainer === 'right' ? 'bg-primary/5 mix-blend-overlay' : ''}`} />
          {focusedContainer === 'right' ? (
            <div className="relative p-6 h-full flex flex-col animate-in fade-in zoom-in duration-300">
              <div className="flex items-center gap-2 mb-4">
                <Sparkles className="w-4 h-4 text-primary animate-pulse" />
                <h3 className="text-sm font-semibold text-white">Generated Document</h3>
              </div>
            ...
            <div className="p-6 h-full space-y-4 animate-in fade-in duration-200">
              <div className="flex items-center gap-2">
                <Skeleton className="h-4 w-4 rounded-full animate-none bg-black/10" />
                <Skeleton className="h-4 w-24 animate-none bg-black/10" />
              </div>
              <div className="space-y-3">
                <Skeleton className="h-4 w-[90%] animate-none bg-black/10" />
                <Skeleton className="h-4 w-[85%] animate-none bg-black/10" />
                <Skeleton className="h-4 w-[88%] animate-none bg-black/10" />
                <Skeleton className="h-4 w-[82%] animate-none bg-black/10" />
              </div>
              <div className="mt-auto pt-4 flex justify-end">
                <Skeleton className="h-3 w-20 animate-none bg-black/10" />
              </div>
            </div>
          )}
        </div>
